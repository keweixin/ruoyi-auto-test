...
class TestDictApi:
    ...
    def test_db_check_dict_type(self, dict_client):
        ...
        try:
            row = db_utils.query_one(
                "SELECT name, type, status, deleted FROM system_dict_type WHERE type=%s",
                (type_,)
            )
        ...
        row2 = db_utils.query_one(
            "SELECT deleted FROM system_dict_type WHERE type=%s", (type_,)
        )
    ...
    def test_db_check_dict_data(self, dict_client):
        ...
        try:
            row = db_utils.query_one(
                "SELECT label, value, dictType, deleted FROM system_dict_data WHERE id=%s",
                (data_id,)
            )
