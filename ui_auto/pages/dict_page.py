...
class DictPage(BasePage):
    ...
    DATA_URL = "/system/dict/data"  # 字典数据页路由
    ...
    def open_data_page(self):
        """打开字典数据页。"""
        self.open(cfg.web_url + self.DATA_URL)
        self.wait_visible(self.page.locator(".el-table"))
    ...
    def add_type(self, name, type_):
        ...
        dialog.get_by_label("字典名称").fill(name)
        dialog.get_by_label("字典类型").fill(type_)
        # 状态默认开启，备注可选
    ...
    def edit_row(self, keyword, new_name=None):
        ...
        dialog = self.page.locator(".el-dialog").first
        if new_name:
            name_input = dialog.get_by_label("字典名称")
            name_input.fill("")
